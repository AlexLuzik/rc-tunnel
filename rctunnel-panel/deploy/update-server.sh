...
for a in "$@"; do case "$a" in
  --check) CHECK_ONLY=1 ;;
  --yes|-y) ASSUME_YES=1 ;;
  *) echo "unknown option: $a (use --check or --yes)" >&2; exit 2 ;;
esac; done
...
run() {
  ...
  # ---- versions (best-effort: never let this kill the run) ------------------
  set +e
  ...
  NEW_SHA="$(git -C "$PANEL_SRC" rev-parse --short HEAD 2>/dev/null)"
...
# Run everything through tee so the log is captured AND the terminal always sees
# the output flushed (process-substitution logging can drop output on a fast exit).
run "$@" 2>&1 | tee -a "$LOG"
exit "${PIPESTATUS[0]}"
